...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(twit: Tweet) = twit
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedAcc(twit: Tweet) = {
    if (elem.retweets > twit.retweets)
      right.mostRetweetedAcc(elem)
    else left.mostRetweetedAcc(twit)
  }
...
object GoogleVsApple {
  ...
  val data = TweetReader.allTweets

  lazy val googleTweets: TweetSet = data.filter(tw => google.contains(tw.text))
  lazy val appleTweets: TweetSet = data.filter(tw => apple.contains(tw.text))
